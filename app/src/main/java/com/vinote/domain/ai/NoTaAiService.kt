...
""".trimIndent()
        ...
        if (result.isSuccess) {
            try {
                ...
                Log.w("NoTaAiService", "Failed to parse OpenRouter JSON response, falling back to offline NLP", e)
            }
        }
    ...
    private fun generateOfflineAssistantResponse(
        ...
        val (reply, chips) = when {
            ...
            lower.contains("nabung") || lower.contains("save") || lower.contains("goal") -> {
                ...
                val summary = if (expenseCount > 0) {
                    "Saat ini kamu sudah mencatat **$expenseCount transaksi pengeluaran** dengan total **${com.vinote.ui.components.FormatUtils.formatRupiah(totalExpense)}**. Ada hal spesifik tentang keuanganmu yang ingin kamu diskusikan?"
                } else {
                    "Aku NoTa 🌟 Siap membantu analisis keuanganmu! Coba tanya seputar saldo, budget harian, atau alokasi tabungan."
                }
                summary to listOf("Saldo aku berapa?", "Kategori pengeluaran", "Saran budget")
            }
        }
